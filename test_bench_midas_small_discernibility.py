...
resolutions=[100,150,200,250,300,350,400]
...
for resolutions_index in range(len(resolutions)):
    for areaOfInterest_thresholds_index in range(len(areaOfInterest_thresholds)):
        ...
        cap = cv2.VideoCapture('CS_thirdfloor_leftwall.mp4', apiPreference=cv2.CAP_MSMF)
        ...
        while num_of_frames < num_of_frames_limit:
            ...
            if ret == False:
                break
            if ret == True:
                if num_of_frames % 1 != 0 or num_of_frames < 200:
...
writer = pd.ExcelWriter('discernability_results.xlsx', engine='xlsxwriter')
